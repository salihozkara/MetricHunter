...
<!-- File     : barchart.xsl                                                     -->
<!-- Usage    : xt sm_out.xml barchart.xsl chart.svg                             -->
<!-- Requires : XT and IE SVG plugin from www.adobe.com/svg                      -->
<!-- Input    : XML output of JavaNCSS or SourceMonitor tool.                    -->
<!-- Output   : Frequency chart METRIC v UNIT in SVG format.                     -->
<!-- History  : 16/08/02 Y. Coene (yves.coene13@yucom.be)                        -->
<!--                   Original for JavaNCSS output.  Shows table in SVG format. -->
<!--            17/07/04                                                         -->
<!--                   Entity section and global variables updated for use with  -->
<!--                   SourceMonitor XML detailed output.                        -->
<!--            04/03/09  Jonas Larsson (geldar@gmail.com)                       -->
<!--                   added namespaces and made xsl more standard compliant     -->
<!--                   Now works in FireFox out of the box w/o adobe svg         -->

<!-- Note: only works if metric values are numeric, i.e. 1234.56 should not be   -->
<!--       encoded as 1,234.56 in the XML input.                                 -->

<!DOCTYPE BARCHART [
        <!ENTITY UNIT        "//files/file/metrics">
        <!ENTITY METRIC      "//files/file/metrics/metric[@id='M1']">
        <!ENTITY SHORTMETRIC "metric[@id='M1']">
        ]>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg" version="1.0">
    ...
    <xsl:variable name="labelx">Files</xsl:variable>
    <xsl:variable name="labely">
        <xsl:value-of select="//metric_name[@id='M1']"/>
    </xsl:variable>
    <xsl:variable name="title1">Distribution
        <xsl:value-of select="$labely"/>
    </xsl:variable>
    <xsl:variable name="title2">per File</xsl:variable>
    ...
    <xsl:template match="/">
        ...
        <svg xmlns="http://www.w3.org/2000/svg"
             width="436pt" height="327pt" viewBox="0 0 4360 3270">
            <rect x="40" y="30" width="4320" height="3240" fill="silver"/>
            <rect x="3" y="3" width="4314" height="3234" fill="white" stroke-width="6px" stroke="black"/>
            <g fill="#FFFFFF" stroke-width="15px" stroke="black">
                <rect x="40" y="40" width="4240" height="3160"/>
            </g>

            <g fill="#000080" font-family="Times New Roman" font-style="italic" font-size="180px" text-anchor="end">
                <text id="Heading" x="4100" y="256">
            ...
            <g transform="translate(190 640) rotate(-90)" fill="#008080" font-family="Times New Roman" font-size="100px"
               text-anchor="end">
                <text>
                    <xsl:value-of select="$labely"/>
                </text>
            </g>
            <g fill="#008000" font-family="Arial" font-size="100px">
                <text id="Notes" x="2535" y="1080">Average
                    <xsl:value-of select="$labely"/> =
                    <xsl:value-of select="format-number(sum(&METRIC;) div count(&UNIT;),'0.00')"/>
                </text>
                <text x="2535" y="1200">Total
                    <xsl:value-of select="$labelx"/> =
                    <xsl:value-of select="count(&UNIT;)"/>
                </text>
                <text x="2535" y="1320">Total
                    <xsl:value-of select="$labely"/> =
                    <xsl:value-of select="sum(&METRIC;)"/>
                </text>

            </g>
            <text x="4100" y="3110" fill="#008080" font-family="Times New Roman" font-size="100px" text-anchor="end">
                <xsl:value-of select="$labelx"/>
            </text>

            <g font-family="Times New Roman" font-size="80px" text-anchor="middle">
    ...
    <xsl:template name="ITEM">
        ...
        <path stroke="#000000" stroke-width="6px" d="M {($x0)-30},{2842-$i*$dy} L {($x0)-1},{2842-$i*$dy} z"/>

        <!-- label on vertical axis -->
        <g font-family="Times New Roman" font-size="80px" text-anchor="end">
        ...
        <g fill="#DBDBED" stroke-width="4px" stroke="black">
    ...
    <xsl:template name="LOOP">
        ...
        <xsl:variable name="cnt">
            <xsl:choose>
                <xsl:when test="$n &lt; $last">
                    <xsl:value-of
                            select="count(&UNIT;[&SHORTMETRIC; >= $step_value*$n and &SHORTMETRIC; &lt; $step_value*($n+1) ])"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:call-template name="ITEM">
            ...
            <xsl:with-param name="x">
                <xsl:value-of select="$step_value*$n"/>
            </xsl:with-param>
            <xsl:with-param name="y">
                <xsl:value-of select="$cnt"/>
            </xsl:with-param>
            <xsl:with-param name="p">
                <xsl:value-of select="$cnt div count(&UNIT;)"/>
            </xsl:with-param>
        </xsl:call-template>
